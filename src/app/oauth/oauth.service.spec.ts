...
class FakeNonceGenerator implements RandomStringGenerator
{
...
class FakeCodeVerifierGenerator implements RandomStringGenerator
{
    public generate(): string {
        return 'very-fire';
    }
...
{
    const CODE_VERIFIER = new FakeCodeVerifierGenerator();
    ...
    describe('#buildRequest', () =>
    {
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, DEFAULT_PROVIDER, DEFAULT_OPTIONS).buildRequest();
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, {}, {}).buildRequest(DEFAULT_OPTIONS, DEFAULT_PROVIDER);
            const options = request.getOptions();
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, null, DEFAULT_OPTIONS).buildRequest({response_type: 'id_token', client_id: 'a-client'});
            const options = request.getOptions();
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, DEFAULT_PROVIDER, null).buildRequest(
                null,
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, {}, {}).buildRequest();
            const options = request.getOptions();
        ...
        {
            const request = new OAuthService(null, CODE_VERIFIER, NONCE, {}, {}).buildRequest({nonce: 'nonsense'});
            const options = request.getOptions();
